...
impl ffi::SmushClient {
    ...
    pub fn remove_timers(&self, index: PluginIndex, group: &QString) -> usize {
        self.rust()
            .client
            .remove_senders::<Timer>(index, &String::from(group))
    }

    pub fn remove_triggers(&self, index: PluginIndex, group: &QString) -> usize {
        self.rust()
            .client
