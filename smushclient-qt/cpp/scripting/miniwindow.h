...
public:
  ...
  MiniWindow(QWidget *parent, const QPoint &location, const QSize &size,
             Position position, Flags flags, const QColor &fill,
             const std::string &pluginID = std::string());
  Hotspot *addHotspot(std::string_view hotspotID, WorldTab *tab,
                      const Plugin *plugin, Hotspot::Callbacks &&callbacks);
  ...
  void drawEllipse(const QRectF &rect, const QPen &pen,
                   const QBrush &brush = QBrush());
  void drawFrame(const QRectF &rect, const QColor &color1,
                 const QColor &color2);
  void drawGradient(const QRectF &rect, const QGradient &gradient);
  void drawImage(const QPixmap &image, const QRectF &rect,
                 const QRectF &sourceRect = QRectF(),
                 DrawImageMode = DrawImageMode::Copy, qreal opacity = 1);
  void drawLine(const QLineF &line, const QPen &pen);
  void drawPolygon(const QPolygonF &polygon, const QPen &pen,
                   const QBrush &brush, Qt::FillRule fillRule);
  void drawPolyline(const QPolygonF &polygon, const QPen &pen);
  void drawRect(const QRectF &rect, const QPen &pen,
                const QBrush &brush = QBrush());
  void drawRoundedRect(const QRectF &rect, qreal xRadius, qreal yRadius,
                       const QPen &pen, const QBrush &brush = QBrush());
  QRectF drawText(const QFont &font, const QString &text, const QRectF &rect,
                  const QColor &color);
  ...
  void reset();
  void setPosition(const QPoint &location, Position position,
...
private:
  ...
  static inline QRect normalizeRect(const QRect &rect,
    ...
    return QRect(x, y, w > 0 ? w : pixmap.width() - w - x,
  ...
  static inline QRectF normalizeRect(const QRectF &rect,
    ...
    qreal x, y, w, h;
    rect.getRect(&x, &y, &w, &h);
    return QRectF(x, y, w > 0 ? w : pixmap.width() - w - x,
                  h > 0 ? h : pixmap.height() - h - y);
  }
