...
QString fmtBadReturn(const Plugin &plugin, string_view routine, int idx,
                     const char *type) {
...
class Benchmarker {
public:
  Benchmarker(const char *name) : name(name) { timer.start(); }
  ~Benchmarker() {
...
static int L_DatabaseOpen(lua_State *L) {
  API("DatabaseOpen")
  expectMaxArgs(L, 3);
  lua_pushinteger(L, getApi(L).DatabaseOpen(qlua::getString(L, 1),
                                            qlua::getString(L, 2),
                                            qlua::getInt(L, 3, 6)));
  return 1;
}
...
static int L_GetLineInfo(lua_State *L) {
  ...
  expectMaxArgs(L, 2);
  qlua::pushQVariant(
      L, getApi(L).GetLineInfo(qlua::getInt(L, 1) - 1, qlua::getInt(L, 2)));
  return 1;
}
...
static int L_GetStyleInfo(lua_State *L) {
  API("GetStyleInfo")
  expectMaxArgs(L, 3);
  qlua::pushQVariant(L, getApi(L).GetStyleInfo(qlua::getInt(L, 1),
                                               qlua::getInt(L, 2),
                                               qlua::getInt(L, 3)));
  return 1;
}
...
static int L_GetTimerInfo(lua_State *L) {
  API("GetTimerInfo")
  expectMaxArgs(L, 2);
  qlua::pushQVariant(L, getApi(L).GetTimerInfo(getPluginIndex(L),
                                               qlua::getQString(L, 1),
                                               qlua::getInt(L, 2)));
  return 1;
}
...
static int L_WindowInfo(lua_State *L) {
  ...
  expectMaxArgs(L, 2);
  qlua::pushQVariant(
      L, getApi(L).WindowInfo(qlua::getString(L, 1), qlua::getInt(L, 2)));
  return 1;
}
...
inline void insertTextTriples(lua_State *L, ScriptApi &api) {
  int n = lua_gettop(L);
  for (int i = 1; i <= n; i += 3)
    api.ColourTell(qlua::getQColor(L, i), qlua::getQColor(L, i + 1),
...
static int L_Hyperlink(lua_State *L) {
  API("Hyperlink")
  expectMaxArgs(L, 7);
  getApi(L).Hyperlink(qlua::getQString(L, 1), qlua::getQString(L, 2),
                      qlua::getQString(L, 3), qlua::getQColor(L, 4),
                      qlua::getQColor(L, 5), qlua::getBool(L, 6, false),
...
static int L_BroadcastPlugin(lua_State *L) {
  API("BroadcastPlugin")
  expectMaxArgs(L, 2);
  lua_pushinteger(L, getApi(L).BroadcastPlugin(getPluginIndex(L),
                                               qlua::getInt(L, 1),
                                               qlua::getString(L, 2)));
  return 1;
}
...
static int L_CallPlugin(lua_State *L) {
  ...
  if (!pluginRef) [[unlikely]]
    return returnCode(L, ApiCode::NoSuchPlugin,
  ...
  if (lua_getglobal(L2, routine.data()) != LUA_TFUNCTION) {
    lua_pop(L2, 1);
    return returnCode(L, ApiCode::NoSuchRoutine,
                      fmtNoSuchRoutine(plugin, routine));
  }
  ...
  for (int i = 1; i <= nargs; ++i)
    if (!qlua::copyValue(L, L2, i + 2)) {
      lua_settop(L, 0);
      return returnCode(L, ApiCode::BadParameter,
                        fmtBadParam(i - 2, luaL_typename(L, i)));
    }
...
static int L_EnablePlugin(lua_State *L) {
  API("EnablePlugin")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).EnablePlugin(qlua::getString(L, 1),
...
static int L_PluginSupports(lua_State *L) {
  API("PluginSupports")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).PluginSupports(qlua::getString(L, 1),
                                                qlua::getString(L, 2)));
}
...
static int L_AddAlias(lua_State *L) {
  ...
  return returnCode(L, getApi(L).AddAlias(getPluginIndex(L), name, pattern,
                                          text, flags, *script));
}
...
static int L_AddTimer(lua_State *L) {
  ...
  return returnCode(L, getApi(L).AddTimer(getPluginIndex(L), name, hour, minute,
                                          second, text, flags, *script));
}
...
static int L_AddTrigger(lua_State *L) {
  ...
  return returnCode(L, getApi(L).AddTrigger(getPluginIndex(L), name, pattern,
                                            text, flags, color, soundFile,
                                            *script, *target, sequence));
}
...
static int L_DeleteAlias(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  return returnCode(
      L, getApi(L).DeleteAlias(getPluginIndex(L), qlua::getQString(L, 1)));
}

static int L_DeleteAliases(lua_State *L) {
  API("DeleteAliases")
  expectMaxArgs(L, 1);
  lua_pushinteger(L, (int)getApi(L).DeleteAliases(getPluginIndex(L),
                                                  qlua::getQString(L, 1)));
  return 1;
}
...
static int L_DeleteTimer(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  return returnCode(
      L, getApi(L).DeleteTimer(getPluginIndex(L), qlua::getQString(L, 1)));
}

static int L_DeleteTimers(lua_State *L) {
  API("DeleteTimers")
  expectMaxArgs(L, 1);
  lua_pushinteger(L, (int)getApi(L).DeleteTimers(getPluginIndex(L),
                                                 qlua::getQString(L, 1)));
  return 1;
}
...
static int L_DeleteTrigger(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  return returnCode(
      L, getApi(L).DeleteTrigger(getPluginIndex(L), qlua::getQString(L, 1)));
}

static int L_DeleteTriggers(lua_State *L) {
  API("DeleteTriggers")
  expectMaxArgs(L, 1);
  lua_pushinteger(L, (int)getApi(L).DeleteTriggers(getPluginIndex(L),
                                                   qlua::getQString(L, 1)));
  return 1;
}
...
static int L_DoAfter(lua_State *L) {
  ...
  return returnCode(L, getApi(L).DoAfter(getPluginIndex(L), seconds, text,
                                         SendTarget::Command));
}
...
static int L_DoAfterSpeedwalk(lua_State *L) {
  ...
  const QString text = qlua::getQString(L, 2);
  return returnCode(L, getApi(L).DoAfter(getPluginIndex(L), seconds, text,
                                         SendTarget::Speedwalk));
}

static int L_EnableAlias(lua_State *L) {
  API("EnableAlias")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).EnableAlias(getPluginIndex(L),
                                             qlua::getQString(L, 1),
                                             qlua::getBool(L, 2, true)));
}
...
static int L_EnableAliasGroup(lua_State *L) {
  ...
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).EnableAliasGroup(getPluginIndex(L),
                                                  qlua::getQString(L, 1),
                                                  qlua::getBool(L, 2, true)));
}
...
static int L_EnableTimer(lua_State *L) {
  API("EnableTimer")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).EnableTimer(getPluginIndex(L),
                                             qlua::getQString(L, 1),
                                             qlua::getBool(L, 2, true)));
}
...
static int L_EnableTimerGroup(lua_State *L) {
  ...
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).EnableTimerGroup(getPluginIndex(L),
                                                  qlua::getQString(L, 1),
                                                  qlua::getBool(L, 2, true)));
}
...
static int L_EnableTrigger(lua_State *L) {
  API("EnableTrigger")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).EnableTrigger(getPluginIndex(L),
                                               qlua::getQString(L, 1),
                                               qlua::getBool(L, 2, true)));
}
...
static int L_IsAlias(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  return returnCode(
      L, getApi(L).IsAlias(getPluginIndex(L), qlua::getQString(L, 1)));
}
...
static int L_IsTimer(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  return returnCode(
      L, getApi(L).IsTimer(getPluginIndex(L), qlua::getQString(L, 1)));
}
...
static int L_IsTrigger(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  return returnCode(
      L, getApi(L).IsTrigger(getPluginIndex(L), qlua::getQString(L, 1)));
}
...
static int L_MakeRegularExpression(lua_State *L) {
  ...
  expectMaxArgs(L, 1);
  qlua::pushQString(L,
                    ScriptApi::MakeRegularExpression(qlua::getQString(L, 1)));
  return 1;
}
...
static int L_SetTriggerOption(lua_State *L) {
  ...
  if (const auto search = triggerBools.find(optionName);
    ...
    return returnCode(L,
                      getApi(L).SetTriggerOption(plugin, label, search->second,
                                                 qlua::getBool(L, 3)));
...
static int L_PlaySound(lua_State *L) {
  ...
  expectMaxArgs(L, 5);
  return returnCode(
      L, getApi(L).PlaySound(qlua::getInt(L, 1), qlua::getQString(L, 2),
...
static int L_SetVariable(lua_State *L) {
  API("SetVariable")
  expectMaxArgs(L, 2);
  getApi(L).SetVariable(getPluginIndex(L), qlua::getString(L, 1),
                        qlua::getString(L, 2));
...
static int L_TextRectangle(lua_State *L) {
  ...
  return returnCode(
      L, getApi(L).TextRectangle(rect, offset, borderColor, borderWidth,
...
static int L_WindowCircleOp(lua_State *L) {
  ...
  case CircleOp::RoundedRectangle:
    return returnCode(L, getApi(L).WindowRoundedRect(
                             windowName, rect, qlua::getNumber(L, 12, 0),
                             qlua::getNumber(L, 13, 0), *pen, brush));
  case CircleOp::Chord:
  case CircleOp::Pie:
    return returnCode(L, ApiCode::OK);
  }
...
static int L_WindowCreate(lua_State *L) {
  ...
  if (!position) [[unlikely]]
    return returnCode(L, ApiCode::BadParameter);
  return returnCode(L, getApi(L).WindowCreate(getPluginIndex(L), windowName,
                                              location, size, *position, flags,
                                              bg));
}
...
static int L_WindowDrawImage(lua_State *L) {
  ...
  if (!mode)
    return returnCode(L, ApiCode::BadParameter);
  return returnCode(L, getApi(L).WindowDrawImage(windowName, imageID, rect,
                                                 *mode, sourceRect));
}
...
static int L_WindowDrawImageAlpha(lua_State *L) {
  ...
  return returnCode(L, getApi(L).WindowDrawImageAlpha(windowName, imageID, rect,
                                                      opacity, origin));
}
...
static int L_WindowFilter(lua_State *L) {
  ...
  case 7:
    return returnCode(
        L, api.WindowFilter(windowName, BrightnessAddFilter(qlua::getInt(L, 7)),
                            rect));
  case 10:
    return returnCode(
        L, api.WindowFilter(
               windowName,
               BrightnessAddFilter(qlua::getInt(L, 7), Channel::Red), rect));
  case 13:
    return returnCode(
        L, api.WindowFilter(
               windowName,
               BrightnessAddFilter(qlua::getInt(L, 7), Channel::Green), rect));
  case 16:
    return returnCode(
        L, api.WindowFilter(
               windowName,
               BrightnessAddFilter(qlua::getInt(L, 7), Channel::Blue), rect));
  case 19:
    return returnCode(L, api.WindowFilter(windowName, GrayscaleFilter(), rect));
  case 21:
    return returnCode(
        L, api.WindowFilter(windowName,
                            BrightnessMultFilter(qlua::getNumber(L, 7)), rect));
  case 22:
    return returnCode(
        L,
        api.WindowFilter(
            windowName,
            BrightnessMultFilter(qlua::getNumber(L, 7), Channel::Red), rect));
  case 23:
    return returnCode(
        L,
        api.WindowFilter(
            windowName,
            BrightnessMultFilter(qlua::getNumber(L, 7), Channel::Green), rect));
  case 24:
    return returnCode(
        L,
        api.WindowFilter(
            windowName,
            BrightnessMultFilter(qlua::getNumber(L, 7), Channel::Blue), rect));
  default:
    return returnCode(L, ApiCode::UnknownOption);
  }
}
...
static int L_WindowFont(lua_State *L) {
  ...
  if (!hint) [[unlikely]]
    return returnCode(L, ApiCode::BadParameter);
  return returnCode(L, getApi(L).WindowFont(windowName, fontID, fontName,
                                            pointSize, bold, italic, underline,
                                            strikeout, *hint));
}
...
static int L_WindowFontInfo(lua_State *L) {
  API("WindowFontInfo")
  expectMaxArgs(L, 3);
  qlua::pushQVariant(L, getApi(L).WindowFontInfo(qlua::getString(L, 1),
                                                 qlua::getString(L, 2),
                                                 qlua::getInt(L, 3)));
  return 1;
}
...
static int L_WindowGradient(lua_State *L) {
  ...
  if (mode != Qt::Horizontal && mode != Qt::Vertical) [[unlikely]]
    return returnCode(L, ApiCode::UnknownOption);
  return returnCode(L, getApi(L).WindowGradient(windowName, rect, color1,
                                                color2, (Qt::Orientation)mode));
}
...
static int L_WindowPolygon(lua_State *L) {
  ...
  if (!brushStyle) [[unlikely]]
    return returnCode(L, ApiCode::BrushStyleNotValid);
  return returnCode(
      L, getApi(L).WindowPolygon(
             windowName, *polygon, *pen, QBrush(brushColor, *brushStyle), close,
...
static int L_WindowRectOp(lua_State *L) {
  ...
  switch (*action) {
  case RectOp::Frame:
    return returnCode(L, getApi(L).WindowRect(windowName, rect,
                                              qlua::getQColor(L, 7), QBrush()));
  case RectOp::Fill:
    return returnCode(L, getApi(L).WindowRect(windowName, rect, QPen(),
                                              qlua::getQColor(L, 7)));
  case RectOp::Invert:
    return returnCode(L, getApi(L).WindowInvert(windowName, rect.toRect()));
  case RectOp::Frame3D:
    return returnCode(L, getApi(L).WindowFrame(
                             windowName, rect, qlua::getQColor(L, 7),
                             qlua::getQColor(L, 8, Qt::GlobalColor::black)));
  case RectOp::Edge3D:
    if (optional<MiniWindow::ButtonFrame> frame = qlua::getButtonFrame(L, 7);
        frame)
      return returnCode(L, getApi(L).WindowButton(
                               windowName, rect.toRect(), *frame,
...
static int L_WindowResize(lua_State *L) {
  API("WindowResize")
  expectMaxArgs(L, 4);
  return returnCode(L, getApi(L).WindowResize(qlua::getString(L, 1),
                                              qlua::getQSize(L, 2, 3),
                                              qlua::getQColor(L, 4)));
}
...
static int L_WindowSetZOrder(lua_State *L) {
  ...
  expectMaxArgs(L, 2);
  return returnCode(
      L, getApi(L).WindowSetZOrder(qlua::getString(L, 1), qlua::getInt(L, 2)));
}

static int L_WindowShow(lua_State *L) {
  API("WindowShow")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).WindowShow(qlua::getString(L, 1),
                                            qlua::getBool(L, 2, true)));
}
...
static int L_WindowText(lua_State *L) {
  ...
  const qreal width =
      getApi(L).WindowText(windowName, fontID, text, rect, color);
...
static int L_WindowAddHotspot(lua_State *L) {
  ...
  if (!cursor) [[unlikely]]
    return returnCode(L, ApiCode::BadParameter);
  return returnCode(L, getApi(L).WindowAddHotspot(
                           getPluginIndex(L), windowName, hotspotID, geometry,
                           std::move(callbacks), tooltip, *cursor, flags));
}

static int L_WindowDeleteHotspot(lua_State *L) {
  API("WindowDeleteHotspot")
  expectMaxArgs(L, 2);
  return returnCode(L, getApi(L).WindowDeleteHotspot(qlua::getString(L, 1),
                                                     qlua::getString(L, 2)));
}
...
static int L_WindowDragHandler(lua_State *L) {
  ...
  expectMaxArgs(L, 5);
  return returnCode(L, getApi(L).WindowUpdateHotspot(
...
static int L_WindowMoveHotspot(lua_State *L) {
  ...
  expectMaxArgs(L, 6);
  return returnCode(
      L, getApi(L).WindowMoveHotspot(
             qlua::getString(L, 1), qlua::getString(L, 2),
...
static int L_WindowScrollwheelHandler(lua_State *L) {
  ...
  expectMaxArgs(L, 3);
  return returnCode(
      L, getApi(L).WindowUpdateHotspot(
             getPluginIndex(L), qlua::getString(L, 1), qlua::getString(L, 2),
             Hotspot::CallbacksPartial{.dragMove = nullopt,
                                       .dragRelease = nullopt,
                                       .mouseOver = nullopt,
                                       .cancelMouseOver = nullopt,
                                       .mouseDown = nullopt,
                                       ...
                                       .mouseUp = nullopt,
                                       .scroll =
                                           (string)qlua::getString(L, 3, "")}));
}

// userdata

static int L_noop(lua_State *L) { return returnCode(L, ApiCode::OK); }
...
static const struct luaL_Reg worldlib_meta[] = {
    {"__tostring", L_world_tostring}, {NULL, NULL}};
