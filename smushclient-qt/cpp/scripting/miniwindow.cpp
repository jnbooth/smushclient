...
inline bool isNotSpace(char c) { return !std::isspace(c); }
...
constexpr QRect calculateGeometry(MiniWindow::Position pos, const QSize &parent,
...
MiniWindow::Painter::Painter(MiniWindow *window)
    : QPainter(&window->pixmap), window(window) {}
...
MiniWindow::MiniWindow(QWidget *parent, const QPoint &location,
                       const QSize &size, Position position, Flags flags,
                       const QColor &fill, const std::string &pluginID)
    : QWidget(parent), background(fill), dimensions(size), flags(flags),
      installed(QDateTime::currentDateTime()), location(location), pixmap(size),
      pluginID(pluginID), position(position) {
  setAttribute(Qt::WA_OpaquePaintEvent);
  pixmap.fill(background);
  applyFlags();
}

Hotspot *MiniWindow::addHotspot(string_view hotspotID, WorldTab *tab,
                                const Plugin *plugin,
...
void MiniWindow::drawButton(const QRect &rectBase, ButtonFrame frameType,
...
void MiniWindow::drawEllipse(const QRectF &rect, const QPen &pen,
                             const QBrush &brush) {
  Painter(this, pen, brush).drawEllipse(normalizeRect(rect));
}

void MiniWindow::drawFrame(const QRectF &rectBase, const QColor &color1,
                           const QColor &color2) {
...
void MiniWindow::drawImage(const QPixmap &image, const QRectF &rectBase,
                           const QRectF &sourceRectBase, DrawImageMode mode,
                           qreal opacity) {
  Painter painter(this);
...
void MiniWindow::drawPolygon(const QPolygonF &polygon, const QPen &pen,
                             const QBrush &brush, Qt::FillRule fillRule) {
...
void MiniWindow::drawRect(const QRectF &rect, const QPen &pen,
                          const QBrush &brush) {
  Painter(this, pen, brush).drawRect(normalizeRect(rect));
}

void MiniWindow::drawRoundedRect(const QRectF &rect, qreal xRadius,
                                 qreal yRadius, const QPen &pen,
                                 const QBrush &brush) {
  Painter(this, pen, brush)
      .drawRoundedRect(normalizeRect(rect), xRadius, yRadius);
}

QRectF MiniWindow::drawText(const QFont &font, const QString &text,
...
void MiniWindow::setPosition(const QPoint &loc, Position pos,
...
void MiniWindow::setZOrder(int order) noexcept { zOrder = order; }

void MiniWindow::updatePosition() {
  const QRect geometry =
      flags.testFlag(Flag::Absolute)
          ? QRect(location, dimensions)
          : calculateGeometry(position, getParentWidget(this)->size(),
                              dimensions);
