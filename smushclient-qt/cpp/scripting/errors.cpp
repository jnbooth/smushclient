...
{
  return ScriptApi::tr("PANIC: unprotected error in call to Lua API: %1")
    .arg(getError(L));
}
...
constexpr ApiCode
convertClientResultCode(int code) noexcept
{
  switch (code) {
    ...
    default:
      return code <= (int)SenderAccessResult::LabelConflict ? Conflict
                                                            : ApiCode::OK;
  }
}
...
{
  return convertClientResultCode<ApiCode::AliasNotFound,
                                 ApiCode::AliasAlreadyExists>(code);
}

ApiCode
convertTimerCode(int code) noexcept
{
  return convertClientResultCode<ApiCode::TimerNotFound,
                                 ApiCode::TimerAlreadyExists>(code);
}

ApiCode
convertTriggerCode(int code) noexcept
{
  return convertClientResultCode<ApiCode::TriggerNotFound,
                                 ApiCode::TriggerAlreadyExists>(code);
}
