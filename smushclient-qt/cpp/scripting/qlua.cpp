...
QLine qlua::getQLine(lua_State *L, int idxX1, int idxY1, int idxX2, int idxY2) {
  return QLine(getInt(L, idxX1), getInt(L, idxY1), getInt(L, idxX2),
               getInt(L, idxY2));
}

QLineF qlua::getQLineF(lua_State *L, int idxX1, int idxY1, int idxX2,
                       int idxY2) {
  return QLineF(getNumber(L, idxX1), getNumber(L, idxY1), getNumber(L, idxX2),
                getNumber(L, idxY2));
}

QMargins qlua::getQMargins(lua_State *L, int idxLeft, int idxTop, int idxRight,
                           int idxBottom) {
  return QMargins(getInt(L, idxLeft), getInt(L, idxTop), getInt(L, idxRight),
                  getInt(L, idxBottom));
}

QMarginsF qlua::getQMarginsF(lua_State *L, int idxLeft, int idxTop,
                             int idxRight, int idxBottom) {
  return QMarginsF(getNumber(L, idxLeft), getNumber(L, idxTop),
                   getNumber(L, idxRight), getNumber(L, idxBottom));
}
...
QRect qlua::getQRect(lua_State *L, int idxLeft, int idxTop, int idxRight,
                     int idxBottom) {
  return QRect(QPoint(getInt(L, idxLeft), getInt(L, idxTop)),
               QPoint(getInt(L, idxRight), getInt(L, idxBottom)));
}

QRectF qlua::getQRectF(lua_State *L, int idxLeft, int idxTop, int idxRight,
                       int idxBottom) {
...
qlua::getButtonFrame(lua_State *L, int idx,
                     optional<MiniWindow::ButtonFrame> ifNil) {
  return getEnum<MiniWindow::ButtonFrame, MiniWindow::ButtonFrame::Raised,
...
qlua::getDrawImageMode(lua_State *L, int idx,
                       optional<MiniWindow::DrawImageMode> ifNil) {
  return getEnum<MiniWindow::DrawImageMode, MiniWindow::DrawImageMode::Copy,
                 MiniWindow::DrawImageMode::CopyTransparent>(L, idx, ifNil);
}

optional<QFont::StyleHint> qlua::getFontHint(lua_State *L, int idx,
                                             optional<QFont::StyleHint> ifNil) {
  if (!checkIsSome(L, idx, LUA_TNUMBER, "integer"))
    return ifNil;
...
optional<QPen> qlua::getPen(lua_State *L, int idxColor, int idxStyle,
                            int idxWidth) {
...
optional<SendTarget> qlua::getSendTarget(lua_State *L, int idx,
                                         optional<SendTarget> ifNil) {
  return getEnum<SendTarget, SendTarget::World, SendTarget::ScriptAfterOmit>(
      L, idx, ifNil);
}

optional<MiniWindow::Position>
qlua::getWindowPosition(lua_State *L, int idx,
                        optional<MiniWindow::Position> ifNil) {
  return getEnum<MiniWindow::Position, MiniWindow::Position::OutputStretch,
