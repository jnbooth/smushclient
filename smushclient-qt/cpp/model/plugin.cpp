...
bool PluginModel::reinstall(const QModelIndex &index)
{
  ...
  {
    emit dataChanged(createIndex(row, 0), createIndex(row, numColumns - 1), {Qt::DisplayRole});
...
bool PluginModel::hasChildren(const QModelIndex &) const
{
  return false;
}
...
{
  if (row < 0 || count <= 0 || row + count > pluginCount || parent.constInternalPointer())
    return false;
