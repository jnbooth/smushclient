...
PluginModel::PluginModel(SmushClient &client, QObject *parent)
    : QAbstractItemModel(parent), client(client),
...
PluginDetails PluginModel::pluginDetails(const QModelIndex &index) const {
  return PluginDetails(&client, pluginId(index));
}

QString PluginModel::pluginId(const QModelIndex &index) const {
  return client.pluginId(pluginIndex(index.row()));
}

bool PluginModel::reinstall(const QModelIndex &index) {
  ...
  emit pluginOrderChanged();
  emit dataChanged(createIndex(newRow, 0), createIndex(newRow, numColumns - 1),
                   {Qt::DisplayRole});
  emit pluginScriptChanged(newIndex);
...
bool PluginModel::hasChildren(const QModelIndex &) const { return false; }
...
QModelIndex PluginModel::index(int row, int column,
...
QMap<int, QVariant> PluginModel::itemData(const QModelIndex &index) const {
  if (!isValidIndex(index))
...
bool PluginModel::isValidIndex(const QModelIndex &index) const noexcept {
