...
TimerModel::TimerModel(SmushClient &client, Timekeeper *timekeeper,
                       QObject *parent)
...
QString TimerModel::exportXml() const { return client.exportWorldTimers(); }
...
const std::array<QString, 4> &TimerModel::headers() const noexcept {
  const static std::array<QString, 4> headers{tr("Group/Label"), tr("Type"),
                                              tr("Occurrence"), tr("Text")};
  return headers;
}
...
void TimerModel::prepareRemove(SenderMap *senderMap, const rust::String &group,
                               int row, int count) {
