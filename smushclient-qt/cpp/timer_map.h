...
template <typename T> class TimerMap : public QObject {
private:
...
public:
  ...
  inline TimerMap(Parent *parent, HandlerSlot<Parent> slot,
                  Qt::TimerType timerType = Qt::TimerType::CoarseTimer)
      : QObject(parent), slot((HandlerSlot<QObject>)slot),
        timerType(timerType) {}
  ...
  template <class Predicate> inline size_t erase_if(Predicate pred) {
