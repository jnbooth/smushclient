...
PrefsTriggers::PrefsTriggers(const World &world, TriggerModel *model,
                             QWidget *parent)
    : AbstractPrefsTree(model, parent), ui(new Ui::PrefsTriggers) {
  ui->setupUi(this);
  setTree(ui->tree);
  CONNECT_WORLD(EnableTriggers);
}

PrefsTriggers::~PrefsTriggers() { delete ui; }
