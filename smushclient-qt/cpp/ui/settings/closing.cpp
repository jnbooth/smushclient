...
SettingsClosing::SettingsClosing(Settings &settings, QWidget *parent)
    : QWidget(parent), ui(new Ui::SettingsClosing) {
  ui->setupUi(this);
  ...
  connect(ui->ConfirmQuit, &QRadioButton::toggled, &settings,
          &Settings::setConfirmQuit);

  EnumButtonGroup(this, settings.getWorldCloseBehavior(), &settings,
                  &Settings::setWorldCloseBehavior)
      .addButton(ui->WorldCloseBehavior_Save,
                 Settings::WorldCloseBehavior::Save)
...
SettingsClosing::~SettingsClosing() { delete ui; }
