...
MudInput::MudInput(QWidget *parent) : QTextEdit(parent), history() {}

MudInput::MudInput(QWidget *parent, const QStringList &history)
    : QTextEdit(parent), history(history) {}

MudInput::MudInput(const QStringList &history)
    : QTextEdit(), history(history) {}

const QStringList &MudInput::log() const noexcept { return history.log(); }

void MudInput::setLog(const QStringList &log) { history.replace(log); }
...
QSize MudInput::sizeHint() const { return minimumSizeHint(); }

// Public slots

void MudInput::clearLog() { history.clear(); }

void MudInput::remember(const QString &text) { history.push(text); }

void MudInput::setIgnoreKeypad(bool ignore) { ignoreKeypad = ignore; }

void MudInput::setMaxLogSize(int size) { history.setMaxSize(size); }
