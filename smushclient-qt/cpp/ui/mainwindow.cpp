...
MainWindow::MainWindow(Notepads *notepads, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), notepads(notepads) {
  ...
  connect(ui->action_close_window, &QAction::triggered, this,
          &MainWindow::close);
  connect(ui->action_log_session, &QAction::triggered, &settings,
          &Settings::setLoggingEnabled);
  connect(ui->action_maximize, &QAction::triggered, this,
          &MainWindow::showMaximized);
  connect(ui->action_minimize, &QAction::triggered, this,
          &MainWindow::showMinimized);
  connect(ui->action_restore, &QAction::triggered, this,
          &MainWindow::showNormal);
  connect(ui->action_status_bar, &QAction::triggered, &settings,
          &Settings::setShowStatusBar);
  connect(ui->action_wrap_output, &QAction::triggered, &settings,
          &Settings::setOutputWrapping);

  recentFileActions =
      QList<QAction *>{ui->action_rec_1, ui->action_rec_2, ui->action_rec_3,
                       ui->action_rec_4, ui->action_rec_5};
...
void MainWindow::closeEvent(QCloseEvent *event) {
  ...
  if (settings.getConfirmQuit() &&
      QMessageBox::question(this, QString(), tr("Close all worlds and quit?"),
...
void MainWindow::on_action_connect_triggered() { worldtab()->connectToHost(); }
...
void MainWindow::on_action_go_to_line_triggered() {
  ...
  const int choice =
      QInputDialog::getInt(this, tr("Go to line"), tr("Enter line number"),
...
void MainWindow::on_action_open_world_triggered() {
  const QString filePath = QFileDialog::getOpenFileName(
      this, ui->action_open_world->text(), QStringLiteral(WORLDS_DIR),
      FileFilter::world());
...
void MainWindow::on_action_paste_triggered() { worldtab()->ui->input->paste(); }
...
void MainWindow::on_action_quit_triggered() { QCoreApplication::quit(); }
...
void MainWindow::on_action_wrap_output_triggered(bool checked) {
  const QTextEdit::LineWrapMode mode =
      checked ? QTextEdit::LineWrapMode::WidgetWidth
...
void MainWindow::on_world_tabs_currentChanged(int index) {
  ...
  socketConnection = connect(activeTab, &WorldTab::connectionStatusChanged,
