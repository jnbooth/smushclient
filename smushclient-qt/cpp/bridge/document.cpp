...
void Document::appendHtml(const QString &html) const { api->appendHtml(html); }
...
void Document::createMxpStat(const QString &entity, const QString &caption,
...
void Document::handleServerStatus(const QByteArray &variableBytes,
...
void Document::handleTelnetNaws() const { api->sendNaws(); }

void Document::handleTelnetNegotiation(TelnetSource source, TelnetVerb verb,
                                       uint8_t code) {
...
void Document::handleTelnetSubnegotiation(uint8_t code,
...
public:
  AliasCallback(rust::Str callback, rust::Str senderName, rust::Str line,
                const rust::Vec<rust::Str> &wildcards,
                const rust::Vec<NamedWildcard> &namedWildcards)
      : DynamicPluginCallback(callback), senderName(senderName), line(line),
        wildcards(wildcards), namedWildcards(namedWildcards) {}
...
public:
  TriggerCallback(rust::Str callback, rust::Str senderName, rust::Str line,
                  const rust::Vec<rust::Str> &wildcards,
                  const rust::Vec<NamedWildcard> &namedWildcards,
...
void Document::send(const SendScriptRequest &request) const {
  if (request.output.empty()) {
    AliasCallback aliasCallback(request.script, request.label, request.line,
                                request.wildcards, request.namedWildcards);
    api->sendCallback(aliasCallback, request.plugin);
  } else {
    TriggerCallback triggerCallback(request.script, request.label, request.line,
                                    request.wildcards, request.namedWildcards,
                                    request.output);
...
void Document::updateMxpStat(const QString &entity,
